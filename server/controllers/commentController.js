...
exports.updateComment = (req, res) => {
    ...
    commentModel.findByIdAndUpdate(
        ...
        { new: true },
        (err, comment) => {
            if (err) res.status(500).json({ message: 'There was an error updating your comment.' });
            else res.status(200).json({ status: `Comment ${req.query.commentID} was successfully updated.`, comment: comment });
        }
    );
};
...
exports.deleteComment = (req, res) => {
    commentModel.findByIdAndDelete(req.query.commentID, (err, comment) => {
        if (err) res.status(500).json({ message: 'There was an error deleting your comment.' });
...
exports.ticketComments = (req, res) => {
    commentModel
        ...
        .sort({ createdAt: -1 })
        .exec((err, comments) => {
            if (err) res.status(500).json({ message: 'There was an error retrieving your comments.' });
