...
exports.newProject = (req, res) => {
    new projectModel({
        ...
        if (err)
            res.status(500).json({
                status: 'An error ocurred while creating your project.',
            });
        else
            res.status(200).json({
...
exports.userProjects = (req, res) => {
    projectModel
        ...
        .sort({createdAt:-1})
        .exec((err, projects) => {
            if (err) res.status(500).json({status: 'An error occurred while finding your projects.'});
...
exports.searchProjects = (req, res) => {
    projectModel
        ...
        .sort({createdAt:'desc'})
        .exec((err, projects) => {
            if (err) res.status(500).json({status: 'An error occurred while searching projects.'});
            else res.status(200).json(projects);
        }
    );
};
...
exports.editProject = (req, res) => {
    projectModel.findOneAndUpdate(
        ...
        (err, project) => {
            if (err) res.status(500).json({ status: 'An error occurred while editing your project.' });
            else res.status(200).json({ status: `Project ID ${req.query.projectID} was edited successfully.` });
        }
    );
};
