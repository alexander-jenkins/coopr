...
exports.newTicket = (req, res) => {
  new ticketModel({
    ...
    if (err)
      res
        .status(500)
        .json({ message: "There was an error creating your ticket." });
    else
      res
        .status(200)
        .json({ status: `Ticket ID ${ticket._id} was created successfully.` });
  });
};
  ...
  ticketModel.findByIdAndUpdate(
    ...
    (err, ticket) => {
      if (err)
        res
          .status(500)
          .json({ status: "There was an error updating your ticket." });
      else
        res.status(200).json({
          status: "Ticket was updated to contain the following new values:",
          ticket: ticket,
        });
    }
  );
...
exports.deleteTicket = (req, res) => {
  ticketModel.findByIdAndDelete(req.query.ticketID, (err, ticket) => {
    if (err)
      res
        .status(500)
        .json({ message: "There was an error deleting your ticket." });
    else {
      //Delete all comments associated with the ticket
      commentModel.deleteMany(
        { ticket: req.query.ticketID },
        (err, comment) => {
          if (err)
            res
              .status(500)
              .json({ message: "There was an error deleting your comments." });
        }
      );
      res.status(200).json({
        status: `Ticket ID ${req.query.ticketID} was deleted successfully.`,
      });
    }
  });
};
